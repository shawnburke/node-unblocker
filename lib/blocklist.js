...
var keywords;// =  new Watchlist( __dirname + "/../keyword-blocklist.txt");

exports.init = function(domainBlocklistPath, keywordBlockListPath) {
  if (domainBlocklistPath) {
    domains = new Watchlist(domainBlocklistPath);
  }

  if (keywordBlockListPath) {
    keywords =  new Watchlist(keywordBlockListPath);
  }
}
...
exports.urlAllowed = function(url){
  ...
  if(domains && domains.data.indexOf(url.hostname) != -1){
  ...
  if(url.hostname) {
		...
		for(; i<= hostname_parts.length-1; i++){
			...
			if(domains && domains.data.indexOf(cur_domain) != -1){
  ...
  if(keywords && keywords.data.some(function(keyword){ 
